...
export default function Home() {
  ...
  const searchEvents = async (city, query) => {
    ...
    try {
      ...
      console.log('Events received:', res.data.results);
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh', paddingBottom: '60px' }}>
      <section style={{ padding: '60px 24px 40px', textAlign: 'center' }}>
        <h1 style={{ fontSize: '3rem', fontWeight: 900, color: '#fff', marginBottom: '16px' }}>
          Discover Events Near You
        </h1>
        <p style={{ color: '#aaa', marginBottom: '32px' }}>
          Search by city – get trending events, save your history, and explore.
        </p>
      ...
      {!loading && selectedCity && events.length === 0 && (
        <p style={{ textAlign: 'center', color: '#666' }}>No events found. Try a different keyword.</p>
      )}
    </div>
  );
}